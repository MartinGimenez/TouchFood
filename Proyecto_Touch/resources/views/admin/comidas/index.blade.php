...
<div class="card-panel black-text">
			<table class="striped centered black-text">
			    ...
			    <tbody>
			    	@foreach($categorias_comidas as $categoria_comida)
			      	<tr>
			        	<td class="agrandarletra">{{ ucwords($categoria_comida->categoria) }}</td>
			        	<form method="POST" action="{{ route('comidas.listado') }}">
              			{!! csrf_field() !!}
              			<input type="hidden" name="categoria" value="{{$categoria_comida->categoria}}">	
